...
def manage_users():
    ...
    st.button("Submit", on_click=validate_user_action, args=(action, username, password, role))
...
def linearity_page():
    ...
    unknown_type = st.selectbox("Unknown type:", ["Unknown concentration","Unknown signal"], key="unknown_type")
    ...
    if conc_input and resp_input:
        try:
            ...
            if len(conc)!=len(resp) or len(conc)==0:
                st.warning("Concentration and response lists must be equal and not empty.")
                return
            ...
            if slope!=0:
                if unknown_type=="Unknown concentration":
            ...
            def export_pdf_linearity():
                content_text = f"Linearity curve:\nEquation: {eq}\nUnknown type: {unknown_type}\nUnknown value: {unknown_value}\nResult: {result:.4f} {unit if unknown_type=='Unknown concentration' else ''}"
...
def sn_page():
    st.header("📊 Signal-to-Noise (S/N) Analysis")
    st.write(f"You are logged in as **{st.session_state.username}**")
...
S/N in concentration: {sn_conc:.4f} {st.session_state.unit if 'unit' in st.session_state else ''}
USP S/N in concentration: {sn_usp_conc:.4f} {st.session_state.unit if 'unit' in st.session_state else ''}"""
...
def main_menu():
    ...
    if role=="admin":
        ...
        choice = st.selectbox("Choose option:", ["Linearity Curve","Signal-to-Noise (S/N)"])
