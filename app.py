...
def generate_pdf_bytes(title, lines, img_bytes=None, logo_path=None):
    ...
    if img_bytes is not None:
        try:
            # write img_bytes to a temp file and embed
...
def login_screen():
    ...
    if submitted:
        ...
        if matched and USERS[matched]["password"] == (password or ""):
            ...
            # let Streamlit rerun automatically
            return
        else:
            st.error(t("invalid"))
...
def sn_panel():
    ...
    # PDF
        ...
        if convert_from_bytes is None:
            st.warning("PDF digitizing requires pdf2image + poppler installed. You can still download the original PDF.")
            uploaded.seek(0)
    ...
    if signal is not None:
        ...
        if st.session_state.linear_slope is not None and st.session_state.linear_slope != 0:
            ...
            st.info("Linearity slope not available or zero. Export slope from Linearity panel to compute LOD/LOQ in concentration.")
