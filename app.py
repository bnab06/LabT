# app.py
# LabT — single-file app (linearity + S/N + users + feedback)
# Keep as app.py and run with Streamlit.
import streamlit as st
import json, os, io, tempfile
from datetime import datetime
from pathlib import Path

# basic numeric + plotting
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

# imaging + optional OCR / PDF->PNG
...
# -------------------------
# PDF helper -> PNG (best-effort)
# -------------------------
def pdf_to_pil(uploaded_file):
...
# -------------------------
# small PDF generator for report (optional)
# -------------------------
...
# -------------------------
# OCR helper (extract X,Y pairs if image contains numbers)
# returns DataFrame with X,Y or empty DF
# -------------------------
...
# -------------------------
# Login screen (dropdown of users) — bilingual + powered
# -------------------------
def login_screen():
    header_area()
    cols = st.columns([2,1])
    ...
    if st.button(t("login"), key="login_btn"):
        ...
        if matched and USERS.get(matched,{}).get("password") == (pwd or ""):
            ...
            # no experimental_rerun usage; returning to main_app next render
            return
        else:
            st.error(t("invalid"))
...
# -------------------------
# Admin panel: manage users and access rights
# -------------------------
def admin_panel():
    st.header(t("admin"))
    ...
    with cols[0]:
        ...
        if sel:
            ...
            if st.button("Modifier cet utilisateur", key=f"modify_{sel}"):
                # show editor in expander
                with st.expander(f"Modifier {sel}", expanded=True):
                    new_pwd = st.text_input("Nouveau mot de passe (laisser vide pour conserver)", type="password", key=f"pw_{sel}")
...
# -------------------------
# Linearity panel (keep behavior unchanged)
# -------------------------
def linearity_panel():
    ...
    # bidirectional compute (automatic)
    ...
    if st.button(t("generate_pdf"), key="lin_pdf"):
        if not company or company.strip()=="":
            ...
            lines = [f"Company: {company or 'N/A'}", f"User: {st.session_state.get('user','Unknown')}",
                     f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", f"Slope: {slope:.6f}",
                     f"Intercept: {intercept:.6f}", f"R²: {r2:.6f}"]
            ...
            if pdfb is not None:
                ...
                st.warning("FPDF not available - cannot generate PDF.")

# -------------------------
# S/N panel — works on original image, displays numeric results to the side
# -------------------------
def sn_panel_full():
    st.header(t("sn"))
    ...
    if uploaded is None:
        st.info("Upload a chromatogram (CSV/image) or enable manual mode.")
        return
    ...
    # Image case
        ...
        try:
            ...
            # we do NOT trust OCR for S/N — we compute vertical projection for numeric signal
            # but keep the original image for display & export
            # attempt optional OCR XY to keep scale if present
            df_xy = extract_xy_from_image_pytesseract(orig_img)
            if not df_xy.empty:
                df = df_xy.copy()
            else:
                arr = np.array(orig_img.convert("L"))
                # vertical projection (max) yields chromatogram-like signal
    ...
    # PDF case -> convert to PNG first
    elif name.endswith(".pdf"):
        pil_img, err = pdf_to_pil(uploaded)
        if pil_img is None:
            st.error(err)
            return
    ...
    if x_min == x_max:
        st.warning("Signal plat ou OCR invalide : utilisation d'un axe X artificiel.")
        df["X"] = np.arange(len(df))
        x_min, x_max = 0.0, float(len(df)-1)

    # region selection: two-handle slider (X domain)
    st.subheader(t("noise_region"))
    default_start = x_min + 0.25*(x_max-x_min); default_end = x_min + 0.75*(x_max-x_min)
    try:
        start, end = st.slider(t("select_region"), min_value=float(x_min), max_value=float(x_max),
                               value=(float(default_start), float(default_end)), key="sn_range_slider")
    except Exception as e:
    ...
    if region.shape[0] < 2:
        st.warning("Région trop petite pour estimer le bruit — fallback: utilisation du signal complet.")
        region = df
    ...
    # slope for LOD/LOQ (from linearity or manual)
    ...
    # numeric results displayed to the side (textual)
    colL, colR = st.columns([1,1])
    with colL:
        st.write(f"{t('sn_classic')}: **{sn_classic:.4f}**")
        st.write(f"{t('sn_usp')}: **{sn_usp:.4f}**")
        st.write(f"Peak retention (X): **{peak_x:.4f}**")
        st.write(f"H (height): **{height:.4f}**")
    with colR:
        st.write(f"Noise (std): **{noise_std:.4f}**")
        st.write(f"W (approx FWHM): **{W:.4f}**")
        if slope_val and slope_val != 0:
    ...
    # draw supplementary plot (keeps original image separate) — we plot numeric signal (projection) but the original image above is left intact
    ...
    # peak detection within region using threshold and distance
    ...
    # export PDF report if FPDF available
    if st.button(t("export_sn_pdf"), key="export_sn_pdf_btn"):
        lines = [f"File: {uploaded.name}", f"User: {st.session_state.get('user','Unknown')}",
                 f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", f"{t('sn_classic')}: {sn_classic:.4f}",
                 f"{t('sn_usp')}: {sn_usp:.4f}", f"Peak X: {peak_x:.4f}", f"H: {height:.4f}, Noise: {noise_std:.4f}, W: {W:.4f}"]
    ...
    with st.form("feedback_form", clear_on_submit=True):
        ...
        if submitted:
            if not fb.strip():
                ...
                feeds = load_feedback(); feeds.append({"sender": name or st.session_state.get("user","anonymous"),
                                                       "message": fb, "time": datetime.now().isoformat(), "reply": ""})
                save_feedback(feeds)
                st.success("Feedback envoyé. Merci!")

    if st.session_state.get("role") == "admin":
        st.markdown("---"); st.subheader(t("view_feedback"))
        feeds = load_feedback()
        if not feeds:
...
def main_app():
    ...
    with cols_top[0]:
        if st.button(t("linearity"), key="btn_linearity"): st.session_state["page"]="linearity"
    with cols_top[1]:
        if st.button(t("sn"), key="btn_sn"): st.session_state["page"]="sn"
    with cols_top[2]:
        if role=="admin":
            if st.button(t("admin"), key="btn_admin"): st.session_state["page"]="admin"
    with cols_top[3]:
        if st.button(t("logout"), key="btn_logout"): logout(); return
    ...
    if page == "linearity":
        ...
        if has_access("sn"): sn_panel_full()
