...
def linearity_page():
    ...
    company_name = st.text_input("Company name for PDF / Nom de la compagnie pour PDF :", value="", key="company_name")
    ...
    if conc_input and resp_input:
        try:
            ...
            if len(conc) != len(resp) or len(conc) == 0:
                st.warning("Lists must be same length / Les listes doivent avoir la même taille.")
                return
