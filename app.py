...
TEXTS = {
    "FR": { "app_title":"LabT", "powered":"Powered by BnB", "username":"Utilisateur", "password":"Mot de passe",
            ...
            "digitize_info":"Digitization : OCR tenté si pytesseract installé (best-effort)",
...
def sn_panel():
    ...
    # PDF
        ...
        if convert_from_bytes is None:
            st.warning("PDF digitizing requires pdf2image + poppler installed.")
            uploaded.seek(0)
    ...
    if signal is not None:
        ...
        if st.session_state.linear_slope is not None and st.session_state.linear_slope != 0:
            slope = st.session_state.linear_slope
            sn_conc = (height / slope)  # height / slope
            unit = st.selectbox(t("unit"), ["µg/mL", "mg/mL", "ng/mL"], index=0)
            st.write(f"S/N (concentration) ≈ {sn_conc:.4f} {unit}")
            # LOD/LOQ
            lod = 3.3 * noise_std / slope
            loq = 10 * noise_std / slope
            st.write(f"{t('lod')}: {lod:.4f} {unit}")
            st.write(f"{t('loq')}: {loq:.4f} {unit}")
        else:
            st.info("Linearity slope not available. Export slope from Linearity panel to compute S/N in concentration.")
        ...
        if st.button(t("export_sn_pdf"), key="sn_export_pdf"):
            ...
            if st.session_state.linear_slope is not None and st.session_state.linear_slope != 0:
                ...
                lines.append(f"{t('lod')}: {lod:.4f} {unit}")
                lines.append(f"{t('loq')}: {loq:.4f} {unit}")
