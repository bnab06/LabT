...
# --- Contenu principal (après login) ---
def main_app():
    # ✅ On déplace la configuration de page ici
    st.set_page_config(page_title="LabT", layout="wide")
    ...
    # ⚙️ Ici tu appelles ton contenu principal (les 25 parties)
    show_main_interface()
...
def main():
    if "logged_in" not in st.session_state or not st.session_state["logged_in"]:
        # 👇 Tant qu’on n’est pas connecté, seule la page de login s’affiche
        login_screen()
    else:
        main_app()
